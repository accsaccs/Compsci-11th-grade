 ...
 * October 2014 
 * 
 * class that makes Minimum Rule
 * calculates and graphs the area under the curve according to the  Minimum Rule
 * minimum rule: the height of the rectangle is whichever side of the slice is shorter 
 */
public class MinimumRule extends Riemann 
{
	 ...
	 * 
	 * finds the area of a certain slice (aka rectangle) under a curve according to the minimum rule
	 * minimum rule: the height of the rectangle is whichever side of the slice is shorter 
	 * 
	...
	public double slice(Polynomial p, double leftCord, double rightCord)
	{
		...
		//do this to find which one is the minimum
		if(heightA < heightB)
		{
			height = heightA;
		}
	 ...
	 * 
	 * graphs the slice (which shows the area under the curve but the curve is not on the graph) according to the minimum rule
	 */
	public void slicePlot(org.opensourcephysics.frames.PlotFrame pframe, polyfun.Polynomial poly, double sleft, double sright)
	{
		...
		//do this to find which is the minimum
		if(heightA < heightB)
		{
			height = heightA;
		}
