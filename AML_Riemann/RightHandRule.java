 ...
 * October 2014 
 * 
 * class that makes Right Hand Rule
 * calculates and graphs the area under the curve according to the  Right Hand Rule
 * right hand rule: the height of the rectangle is the height of the right most point
 */
public class RightHandRule extends Riemann 
{
	 ...
	 * 
	 * finds the area of a certain slice (aka rectangle) under a curve according to the right hand rule
	 * right hand rule: the height of the rectangle is the height of the right most point
	 * 
	...
	public double slice(Polynomial p, double leftCord, double rightCord)
	{
		...
		height = p.evaluate(rightCord).getTerms()[0].getTermDouble(); //evaluates the left coordinate to get the y coordinate (height) at that point
		//System.out.println(height);
		
		theSlice = base*height; //area of rectangle formula 
	 ...
	 * 
	 * graphs the slice (which shows the area under the curve but the curve is not on the graph) according to the right hand rule
	 */
	public void slicePlot(org.opensourcephysics.frames.PlotFrame pframe, polyfun.Polynomial poly, double sleft, double sright)
	{
		...
		height = poly.evaluate(sright).getTerms()[0].getTermDouble();
		xcoord = width/2;
		ycoord = height/2;
